...
-- Append speaker id's
SELECT concat(cast(person_id as string), ',', words) as combined
FROM (
  SELECT words, 
         person_id, 
         if(person_id <> lag(person_id) over (order by pos), 1, 0) as speaker_changed,
         row_number() over (order by 1) - 1 as pos
  FROM table
) t
WHERE speaker_changed = 1 or pos = 0
